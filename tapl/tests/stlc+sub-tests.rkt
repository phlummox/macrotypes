...
(typecheck-fail #\c) ; unsupported literal
(typecheck-fail 'yolo) ; unsupported literal
(check-type (λ ([x : Int] [y : Int]) x) : (→ Int Int Int))
(check-not-type (λ ([x : Int]) x) : Int)
(check-type (λ ([x : Int]) x) : (→ Int Int))
(check-type (λ ([f : (→ Int Int)]) 1) : (→ (→ Int Int) Int))
(check-type ((λ ([x : Int]) x) 1) : Int ⇒ 1)
(typecheck-fail ((λ ([x : Foobar]) x) 1)) ; Foobar is not valid type
(typecheck-fail (λ ([x : Foobar]) x)) ; Foobar is not valid type
